...
個人が作ったパチスロ用ツールを持ち寄る場所です。
1本目として、スマスロ 甲鉄城のカバネリ 海門決戦のCZ発光カウンターを公開しています。
他の開発者からのツール掲載も受け付けています（`info@minnanoslot.com`）。
...
**2026-09-09 に本編へ切り替えました。** コンセプトを「みんなのデータで設定判別」から
「個人が作ったパチスロツールを持ち寄る場所」に変更し、送信機能に依存する記述（登録・記録の
引き継ぎ等）は削除済み。他の開発者からのツール掲載を受け付ける導線（`info@minnanoslot.com`
宛のメール）を追加しました。詳しい経緯は `TODO.md` の「AdSense申請前チェックリスト」参照。
