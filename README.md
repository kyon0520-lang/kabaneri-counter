...
kyojiが個人で作ったパチスロ用ツールのポートフォリオサイトです。
現在3作品（カバネリ海門発光カウンター／全台系まつわるチェッカー／
カバネリチャレンジおみくじ）を公開しています。
不具合報告・ご意見は`info@minnanoslot.com`まで。
...
| `img/shot-kabaneri.jpg` `img/shot-matsuwaru.jpg` `img/shot-omikuji.jpg` | トップページの作品カード・カルーセルに使う実機スクリーンショット |
...
**2026-09-16 に「自分（kyoji）が作ったパチスロ用ツールのポートフォリオサイト」として
作り直しました。** doisena.jp（イラストレーターのポートフォリオサイト）の構成・意匠を
参考にしている：白黒・太い輪郭線・輪郭だけの大見出し（CONCEPT/WORK/PROFILE/CONTACT）・
作品カルーセル。掲載ツールは3本（カバネリ海門発光カウンター／全台系まつわるチェッカー／
カバネリチャレンジおみくじ）で、各カードには実機スクリーンショット（`img/shot-*.jpg`）を
端末フレーム風に収めている。詳しい経緯は `TODO.md` の「AdSense申請前チェックリスト」参照。
...
**push すると自動で本番に公開されます。** wrangler.toml の `pages_build_output_dir` の存在と、
無人の自動コミット（まつわるデータ更新ボット）にも毎回デプロイ履歴が対応していることから、
Cloudflare Pages が GitHub 連携で自動デプロイする設定に**すでになっている**と判明した
（2026-09-12。以前ここには「未実施」と書いていたが誤り）。

つまり、**作業ディレクトリに他の未確定な変更が残っていても、commit・push した時点の
コミット内容だけで本番が上書きされる。** 逆に、commit していないファイルの変更は
push しても本番には反映されない。

上の `wrangler pages deploy .` は、これとは別に**ディスク上の今の状態をそのまま**
本番に上げる手動の手段（git の状態と無関係）。両方が有効なので、
「commit していない他の作業が残ったまま `wrangler pages deploy .` を打つ」と、
その未確定な変更まで巻き込んで本番に出てしまう事故が起きうる。deploy 前には
`git status` で作業ディレクトリがきれいか確認すること。
...
| コミットと push | **できるが、push した時点で本番に公開される（上記のとおり）** |
