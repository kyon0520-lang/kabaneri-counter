...
DUP={'カバネリ海門決戦、Sカバネリ':['カバネリ海門','Sカバネリ'],
     ...
     # ここから下は kabuki の連想見出しで複数機種が並記されているもの
     'ﾈｵｱｲﾑｼﾞｬｸﾞﾗｰEX＆ｳﾙﾄﾗﾐﾗｸﾙｼﾞｬｸﾞﾗｰ':['ネオアイム','ウルトラミラジャグ'],
     '北斗の拳・カバネリ・このすば':['スマスロ北斗の拳','カバネリ','このすば'],
     'かぐや様・Lゴッドイーター・からくりサーカス':['かぐや様','ゴッドイーター','からくりサーカス'],
     'ﾆｭｰｷﾝｸﾞﾊﾅﾊﾅ・ｷﾝｸﾞﾊﾅﾊﾅ-30':['ニューキングハナハナ','キンハナ'],
     '異世界カルテット・このすば':['異世界かるてっと','このすば'],
     'ヴヴヴ・ヴヴヴ2':['初代ヴァルヴレイヴ','ヴァルヴレイヴ2'],
     'ゴッドイーター、SBJ':['ゴッドイーター','SBJ']}
# 私の判断で寄せた分（ユーザー確認済み）
AUTO=[['マイジャグV','マイジャグ','マイジャグⅤ','マイジャグラーV'],
      ...
      ['アイム','Sアイム','アイムジャグラー','アイムEX'],['ネオアイム','ネオアイムEX','ネオアイムジャグラーEX','Sネオアイム'],
      ...
      ['モンハンライズ','モンハン'],['アバサー','Sアバサー'],['鏡','スマスロ鏡','L鏡'],['喰霊零','喰霊'],
      ...
      ['ヴヴヴ'],['ヴヴヴ2'],['戦国乙女5'],['L炎炎'],['SAOⅡ'],['ギアスC.C.','ギアスCC','ギアス','コードギアス'],
      ['アニマルドッチ','アニマルスロットドッチ','ドッチ'],
...
for g in AUTO:
    for m in g:
        # 見出しに出ていない表記でも登録しておく（結果欄にしか出ない表記があり、
        # そこから台数を引くときに名寄せが効かないため）
        canon[m]=g[0]
...
# 名寄せの連鎖を解決する（A→B→C のように多段になっている場合、最終形まで辿る）
for m in list(canon):
    cur, seen = canon[m], {m}
    while cur in canon and canon[cur] != cur and cur not in seen:
        seen.add(cur); cur = canon[cur]
    canon[m] = cur
...
for r in recs:
    ...
    if m in DUP:
        for t in DUP[m]:
            x=dict(r); x['machine']=canon.get(t,t); x['category']='機種'; x['note']='並記見出しを分割'; out.append(x)
        continue
    x=dict(r)
    if NON.search(m):
...
_KIND = [
    ...
    (2, r'^\d{1,2}/\d{1,2}|^\d{1,2}月\d{1,2}日|の日$|記念日|誕生日|キャラ誕|BD$|周年|の記念'),
