...
2026-09-09 に本編へ切り替えた。当初は「個人が作ったパチスロツールを持ち寄る場所」という
マーケットプレイス型のコンセプトで書いたが、2026-09-16 に「自分(kyoji)が作った
パチスロ用ツールのポートフォリオサイト」に書き直した。他の開発者からの投稿は前提にせず、
実在する3作品（カバネリ海門発光カウンター／全台系まつわるチェッカー／
カバネリチャレンジおみくじ）を並べる構成。デザインは doisena.jp を参考にした
白黒・太い輪郭線・輪郭だけの大見出し（CONCEPT/WORK/PROFILE/CONTACT）。
送信機能(SEND_ENABLED)の有無とは独立している。
soon はカウンター単体だけを先に出したいときのための切り替えとして残してある。
"""
import os
import sys
